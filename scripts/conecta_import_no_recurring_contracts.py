...
def _parse_line(line):
    return {
        ...
        'recurring_each_order': line[17],
        'recurring_each': line[18],
...
def get_contract_vals(data, idx):
    ...
    if data.get('partner_invoice_extid'):
        ...
        try:
            invoice_partner_id = 7630
            partner_invoice = session.env.ref(data['partner_invoice_extid'])
        except:
            logging.error('OBTENIENDO PARTNER INVOICE %s EN (LIN %s)' % (data['partner_invoice_extid'], str(idx)))
        if partner_invoice:
    ...
    vals = {
        ...
        'invoice_partner_id': invoice_partner_id and invoice_partner_id or partner_id,
        'user_id': user_id,
